...
export default function PrincipalPage() {
  ...
  useEffect(() => {
    const loadProfiles = async () => {
      try {
        setLoading(true);
        // Solo los 3 demos del tutorial (sin mezclar API — hasta lanzamiento no hay más en el feed).
        const activeDemos = TUTORIAL_DEMO_PROFILES.filter((p) => !discardedProfiles.includes(p._id));
        const available = [...activeDemos];
