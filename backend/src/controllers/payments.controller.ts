...
import { createTransporter, getSupportEmail, buildReceiptHtml, ReceiptData } from '../utils/email';
...
export const capturePaymentOrder = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Enviar recibo al email del cliente de forma asíncrona (no bloquear respuesta)
    sendReceiptEmail(payment._id.toString(), req.user.userId).catch((err) =>
      console.error('⚠️ Error enviando recibo automático:', err)
    );
...
/**
 * Lógica interna de envío de recibo, reutilizable desde capturePaymentOrder y el endpoint manual.
 */
const sendReceiptEmail = async (paymentId: string, userId: string): Promise<void> => {
  const payment = await Payment.findOne({
    _id: paymentId,
    userId,
    status: 'completed',
  });
  if (!payment) throw new Error('Pago completado no encontrado');

  const profile = await Profile.findById(payment.profileId);
  const user = await User.findById(userId);
  if (!profile || !user) throw new Error('Perfil o usuario no encontrado');

  const plan = getPlan(payment.planType as PlanType);
  const profileName =
    profile.profileData?.username ||
    (profile.profileData as any)?.channelName ||
    (profile.profileData as any)?.handle ||
    'Mi Perfil';
  ...
  await transporter.sendMail({
    ...
    to: user.email,
    subject: `✅ Recibo de pago – ${receiptData.profileName} | SocialRRSS`,
    html: buildReceiptHtml(receiptData),
  });

  console.log(`✅ Recibo enviado a ${user.email} (pago ${payment._id})`);
};

// Endpoint manual: reenviar recibo por email
export const sendReceipt = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const { paymentId } = req.params;
    await sendReceiptEmail(paymentId, req.user.userId);
    ...
    const msg =
      error.code === 'EAUTH'
        ...
        : error.message || 'Error al enviar el recibo por email';
